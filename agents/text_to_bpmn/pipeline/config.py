...
# config.py lives at agents/text_to_bpmn/pipeline/config.py -> repo root is 4 levels up.
REPO_ROOT = Path(__file__).resolve().parents[3]

# Resources bundled with the text->BPMN agent (terminology config, OMG XSDs) live inside the
# pipeline package so the agent is self-contained and deployable on its own.
