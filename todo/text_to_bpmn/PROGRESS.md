...
uvicorn agents.text_to_bpmn.demo.main:app --reload   # then open http://127.0.0.1:8000
```
...
- **Backend** `agents/text_to_bpmn/demo/main.py` — `POST /api/generate` runs ingest→evidence→IR→
...
- **Frontend** `agents/text_to_bpmn/demo/static/index.html` — self-contained; renders BPMN with bpmn-js
